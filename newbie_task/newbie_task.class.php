...
class plugin_newbie_task_member extends plugin_newbie_task {
	...
	function connect_message($param) {
		$this->register_message($param);
	}
	...
	function register_message($param) {
		...
		//file_put_contents($param['param'][0].'.txt', serialize($_G));

		// 注册完成的时候才会执行下面的内容
		if ($param['param'][0] == 'register_succeed' && $_G['uid'])
		{
			// 取符合条件的任务
